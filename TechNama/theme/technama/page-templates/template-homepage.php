<?php
/**
 * Template Name: Homepage
 * TechNama custom homepage
 */

if (!defined('ABSPATH')) exit;

get_header();
?>

<div class="tn-homepage">

    <?php
    $featured_args = array(
        'posts_per_page'      => 1,
        'post_status'         => 'publish',
        'ignore_sticky_posts' => 1,
        'meta_query'          => array(
            array(
                'key'     => '_thumbnail_id',
                'compare' => 'EXISTS',
            ),
        ),
    );
    $sticky_posts = get_option('sticky_posts');
    if (!empty($sticky_posts) && is_array($sticky_posts)) {
        $featured_args['post__in'] = array_keys($sticky_posts);
    }
    $hero_query = new WP_Query($featured_args);
    ?>

    <section class="hero-section" aria-label="<?php esc_attr_e('Featured Article', 'technama'); ?>">
        <div class="container">
            <?php if ($hero_query->have_posts()) : ?>
                <?php $hero_query->the_post(); ?>
                <?php
                $categories     = get_the_category();
                $cat_name       = !empty($categories) ? $categories[0]->name : '';
                $reading_time   = technama_get_reading_time();
                ?>
                <div class="hero-grid">
                    <a href="<?php the_permalink(); ?>" class="hero-main" aria-label="<?php the_title_attribute(); ?>">
                        <?php if (has_post_thumbnail()) : ?>
                            <?php the_post_thumbnail('technama-hero', array('class' => 'hero-image')); ?>
                        <?php else : ?>
                            <img src="<?php echo esc_url(get_template_directory_uri()); ?>/assets/images/placeholder-hero.svg" alt="<?php the_title_attribute(); ?>" class="hero-image">
                        <?php endif; ?>
                        <div class="hero-overlay">
                            <?php if ($cat_name) : ?>
                                <span class="hero-category"><?php echo esc_html($cat_name); ?></span>
                            <?php endif; ?>
                            <h1 class="hero-title"><?php the_title(); ?></h1>
                            <p class="hero-excerpt"><?php echo esc_html(wp_trim_words(get_the_excerpt(), 25)); ?></p>
                            <div class="hero-meta">
                                <span class="author"><?php echo esc_html(get_the_author()); ?></span>
                                <span class="separator"></span>
                                <time datetime="<?php echo esc_attr(get_the_date('c')); ?>"><?php echo esc_html(get_the_date()); ?></time>
                                <?php if ($reading_time > 0) : ?>
                                    <span class="separator"></span>
                                    <span class="reading-time"><?php printf(esc_html__('%d min read', 'technama'), $reading_time); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </a>

                    <div class="hero-sidebar">
                        <?php
                        $hero_main_id = get_the_ID();
                        $hero_side_args = array(
                            'posts_per_page'      => 2,
                            'post_status'         => 'publish',
                            'ignore_sticky_posts' => 1,
                            'meta_query'          => array(
                                array(
                                    'key'     => '_thumbnail_id',
                                    'compare' => 'EXISTS',
                                ),
                            ),
                        );
                        if (!empty($sticky_posts) && is_array($sticky_posts)) {
                            $hero_side_args['post__in'] = array_values(array_diff(array_keys($sticky_posts), array($hero_main_id)));
                        }
                        $hero_side_query = new WP_Query($hero_side_args);

                        while ($hero_side_query->have_posts()) : $hero_side_query->the_post();
                            $side_cats      = get_the_category();
                            $side_cat_name  = !empty($side_cats) ? $side_cats[0]->name : '';
                        ?>
                            <a href="<?php the_permalink(); ?>" class="hero-card" aria-label="<?php the_title_attribute(); ?>">
                                <?php if (has_post_thumbnail()) : ?>
                                    <?php the_post_thumbnail('technama-card', array('class' => 'hero-image')); ?>
                                <?php else : ?>
                                    <img src="<?php echo esc_url(get_template_directory_uri()); ?>/assets/images/placeholder-card.svg" alt="<?php the_title_attribute(); ?>" class="hero-image">
                                <?php endif; ?>
                                <div class="hero-overlay">
                                    <?php if ($side_cat_name) : ?>
                                        <span class="hero-category"><?php echo esc_html($side_cat_name); ?></span>
                                    <?php endif; ?>
                                    <h2 class="hero-title"><?php the_title(); ?></h2>
                                    <div class="hero-meta">
                                        <time datetime="<?php echo esc_attr(get_the_date('c')); ?>"><?php echo esc_html(get_the_date()); ?></time>
                                    </div>
                                </div>
                            </a>
                        <?php endwhile; ?>
                        <?php wp_reset_postdata(); ?>
                    </div>
                </div>
            <?php endif; ?>
            <?php wp_reset_postdata(); ?>
        </div>
    </section>

    <?php if (is_active_sidebar('sponsor-banner')) : ?>
    <section class="sponsor-banner-area" aria-label="<?php esc_attr_e('Sponsors', 'technama'); ?>">
        <div class="container">
            <?php dynamic_sidebar('sponsor-banner'); ?>
        </div>
    </section>
    <?php endif; ?>

    <?php
    $live_video_id = get_theme_mod('technama_live_video_id', '');
    if (!empty($live_video_id)) :
    ?>
    <section class="tn-live-stream" aria-label="<?php esc_attr_e('Live Stream', 'technama'); ?>">
        <div class="container">
            <div class="section-header">
                <h2><?php esc_html_e('Live Stream', 'technama'); ?></h2>
            </div>
            <div class="tn-live-stream-wrapper">
                <div class="tn-live-stream-video">
                    <iframe
                        src="https://www.youtube.com/embed/<?php echo esc_attr($live_video_id); ?>?autoplay=0&rel=0"
                        title="<?php esc_attr_e('Live Stream', 'technama'); ?>"
                        frameborder="0"
                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                        allowfullscreen
                        loading="lazy">
                    </iframe>
                </div>
            </div>
        </div>
    </section>
    <?php endif; ?>

    <div class="container">
        <div class="tn-homepage-layout">

            <div class="tn-homepage-content">

                <?php
                $latest_args = array(
                    'posts_per_page'      => 6,
                    'post_status'         => 'publish',
                    'ignore_sticky_posts' => 1,
                );
                $latest_query = new WP_Query($latest_args);
                ?>

                <section class="tn-latest-news" aria-label="<?php esc_attr_e('Latest News', 'technama'); ?>">
                    <div class="section-header">
                        <h2><?php esc_html_e('Latest News', 'technama'); ?></h2>
                        <?php
                        $posts_page_id = get_option('page_for_posts');
                        $view_all_url  = $posts_page_id ? get_permalink($posts_page_id) : get_post_type_archive_link('post');
                        if (!$view_all_url) $view_all_url = home_url('/');
                        ?>
                        <a href="<?php echo esc_url($view_all_url); ?>" class="view-all">
                            <?php esc_html_e('View All', 'technama'); ?>
                        </a>
                    </div>

                    <?php if ($latest_query->have_posts()) : ?>
                        <div class="content-grid">
                            <?php while ($latest_query->have_posts()) : $latest_query->the_post(); ?>
                                <?php
                                $post_categories  = get_the_category();
                                $post_cat_name    = !empty($post_categories) ? $post_categories[0]->name : '';
                                $post_reading_time = technama_get_reading_time();
                                ?>
                                <article class="article-card">
                                    <div class="card-image">
                                        <a href="<?php the_permalink(); ?>" aria-label="<?php the_title_attribute(); ?>">
                                            <?php if (has_post_thumbnail()) : ?>
                                                <?php the_post_thumbnail('technama-card', array('alt' => get_the_title())); ?>
                                            <?php else : ?>
                                                <img src="<?php echo esc_url(get_template_directory_uri()); ?>/assets/images/placeholder-card.svg" alt="<?php the_title_attribute(); ?>">
                                            <?php endif; ?>
                                        </a>
                                        <?php if ($post_cat_name) : ?>
                                            <span class="card-category"><?php echo esc_html($post_cat_name); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="card-body">
                                        <h3 class="card-title">
                                            <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                        </h3>
                                        <div class="card-meta">
                                            <span class="author"><?php echo esc_html(get_the_author()); ?></span>
                                            <span class="date">
                                                <time datetime="<?php echo esc_attr(get_the_date('c')); ?>"><?php echo esc_html(get_the_date()); ?></time>
                                            </span>
                                            <?php if ($post_reading_time > 0) : ?>
                                                <span class="reading-time"><?php printf(esc_html__('%d min read', 'technama'), $post_reading_time); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </article>
                            <?php endwhile; ?>
                        </div>
                    <?php endif; ?>
                    <?php wp_reset_postdata(); ?>
                </section>

                <?php if (is_active_sidebar('homepage-below-hero')) : ?>
                <section class="sponsor-banner-area sponsor-mid-content" aria-label="<?php esc_attr_e('Sponsored', 'technama'); ?>">
                    <?php dynamic_sidebar('homepage-below-hero'); ?>
                </section>
                <?php endif; ?>

                <?php
                $startup_args = array(
                    'posts_per_page'      => 4,
                    'post_status'         => 'publish',
                    'ignore_sticky_posts' => 1,
                    'category_name'       => 'startups',
                );
                $startup_query = new WP_Query($startup_args);

                if ($startup_query->have_posts()) :
                ?>
                <section class="tn-startup-showcase" aria-label="<?php esc_attr_e('Startup Showcase', 'technama'); ?>">
                    <div class="section-header">
                        <h2><?php esc_html_e('Startup Showcase', 'technama'); ?></h2>
                        <?php $startup_cat_id = get_cat_ID('Startups'); ?>
                        <?php if ($startup_cat_id && !is_wp_error($startup_cat_id)) : ?>
                            <a href="<?php echo esc_url(get_category_link($startup_cat_id)); ?>" class="view-all">
                                <?php esc_html_e('View All', 'technama'); ?>
                            </a>
                        <?php endif; ?>
                    </div>

                    <div class="content-grid content-grid-2">
                        <?php while ($startup_query->have_posts()) : $startup_query->the_post(); ?>
                            <?php
                            $startup_cats         = get_the_category();
                            $startup_cat_name     = !empty($startup_cats) ? $startup_cats[0]->name : '';
                            $startup_reading_time = technama_get_reading_time();
                            ?>
                            <article class="article-card article-card-horizontal">
                                <div class="card-image">
                                    <a href="<?php the_permalink(); ?>" aria-label="<?php the_title_attribute(); ?>">
                                        <?php if (has_post_thumbnail()) : ?>
                                            <?php the_post_thumbnail('technama-card', array('alt' => get_the_title())); ?>
                                        <?php else : ?>
                                            <img src="<?php echo esc_url(get_template_directory_uri()); ?>/assets/images/placeholder-card.svg" alt="<?php the_title_attribute(); ?>">
                                        <?php endif; ?>
                                    </a>
                                </div>
                                <div class="card-body">
                                    <?php if ($startup_cat_name) : ?>
                                        <span class="card-category"><?php echo esc_html($startup_cat_name); ?></span>
                                    <?php endif; ?>
                                    <h3 class="card-title">
                                        <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                    </h3>
                                    <p class="card-excerpt"><?php echo esc_html(wp_trim_words(get_the_excerpt(), 15)); ?></p>
                                    <div class="card-meta">
                                        <span class="author"><?php echo esc_html(get_the_author()); ?></span>
                                        <span class="date">
                                            <time datetime="<?php echo esc_attr(get_the_date('c')); ?>"><?php echo esc_html(get_the_date()); ?></time>
                                        </span>
                                    </div>
                                </div>
                            </article>
                        <?php endwhile; ?>
                    </div>
                </section>
                <?php
                endif;
                wp_reset_postdata();
                ?>
            </div>

            <aside class="tn-sidebar tn-homepage-sidebar" role="complementary">

                <?php $newsletter_url = get_theme_mod('technama_newsletter_url', '#'); ?>
                <div class="widget widget_newsletter">
                    <h3 class="widget-title"><?php esc_html_e('Newsletter', 'technama'); ?></h3>
                    <p><?php esc_html_e('Get the latest tech news delivered to your inbox.', 'technama'); ?></p>
                    <form action="<?php echo esc_url($newsletter_url); ?>" method="post" target="_blank" rel="noopener" class="newsletter-form">
                        <input type="email" name="EMAIL" placeholder="<?php esc_attr_e('Your email address', 'technama'); ?>" required aria-label="<?php esc_attr_e('Email address', 'technama'); ?>">
                        <button type="submit" class="btn btn-primary btn-sm"><?php esc_html_e('Subscribe Now', 'technama'); ?></button>
                    </form>
                </div>

                <?php
                $trending_args = array(
                    'posts_per_page'      => 5,
                    'post_status'         => 'publish',
                    'ignore_sticky_posts' => 1,
                    'orderby'             => 'comment_count',
                    'order'               => 'DESC',
                );
                $trending_query = new WP_Query($trending_args);

                if ($trending_query->have_posts()) :
                ?>
                <div class="widget widget_trending">
                    <h3 class="widget-title"><?php esc_html_e('Trending Now', 'technama'); ?></h3>
                    <div class="widget_recent_entries">
                        <?php
                        $trending_index = 0;
                        while ($trending_query->have_posts()) : $trending_query->the_post();
                            $trending_index++;
                        ?>
                            <div class="post-item">
                                <span class="trending-number"><?php echo esc_html($trending_index); ?></span>
                                <div class="post-info">
                                    <h4 class="post-title">
                                        <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                    </h4>
                                    <span class="post-date"><?php echo esc_html(get_the_date()); ?></span>
                                </div>
                            </div>
                        <?php endwhile; ?>
                    </div>
                </div>
                <?php
                wp_reset_postdata();
                endif;
                ?>

                <?php
                $social_platforms = array(
                    'facebook'  => array(
                        'url'   => get_theme_mod('technama_social_facebook', '#'),
                        'label' => 'Facebook',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>',
                    ),
                    'twitter'   => array(
                        'url'   => get_theme_mod('technama_social_twitter', '#'),
                        'label' => 'Twitter',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
                    ),
                    'youtube'   => array(
                        'url'   => get_theme_mod('technama_social_youtube', '#'),
                        'label' => 'YouTube',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>',
                    ),
                    'instagram' => array(
                        'url'   => get_theme_mod('technama_social_instagram', '#'),
                        'label' => 'Instagram',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>',
                    ),
                    'telegram'  => array(
                        'url'   => get_theme_mod('technama_social_telegram', '#'),
                        'label' => 'Telegram',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>',
                    ),
                    'whatsapp'  => array(
                        'url'   => get_theme_mod('technama_social_whatsapp', '#'),
                        'label' => 'WhatsApp',
                        'icon'  => '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>',
                    ),
                );

                $has_social = false;
                foreach ($social_platforms as $social) {
                    if (!empty($social['url']) && $social['url'] !== '#') {
                        $has_social = true;
                        break;
                    }
                }

                if ($has_social) :
                ?>
                <div class="widget widget_social_links">
                    <h3 class="widget-title"><?php esc_html_e('Follow Us', 'technama'); ?></h3>
                    <div class="social-icons">
                        <?php foreach ($social_platforms as $platform => $data) :
                            if (empty($data['url']) || $data['url'] === '#') continue;
                        ?>
                            <a href="<?php echo esc_url($data['url']); ?>"
                               class="social-icon <?php echo esc_attr($platform); ?>"
                               target="_blank"
                               rel="noopener noreferrer"
                               aria-label="<?php echo esc_attr($data['label']); ?>">
                                <?php echo $data['icon']; ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
                <?php endif; ?>

            </aside>

        </div>
    </div>

</div>

<?php get_footer(); ?>
